...
void InitRtpHeader(struct RtpPacket* rtpPacket, uint8_t csrcLen, uint8_t extension, uint8_t padding,
                   uint8_t version, uint8_t payloadType, uint8_t marker, uint16_t seq,
                   uint32_t timestamp, uint32_t ssrc);

int SendRtpPacketOverTcp(int clientSockfd, struct RtpPacket* rtpPacket, uint32_t dataSize);

int SendRtpPacketOverUdp(int serverRtpSockfd, const char* ip, int16_t port,
